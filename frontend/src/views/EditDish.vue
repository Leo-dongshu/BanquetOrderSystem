  ...
  <div class="edit-dish">
    <el-card>
      ...
      <div class="edit-dish-content">
        <el-form :model="dishForm" label-width="120px" @submit.prevent="updateDish">
          ...
          <el-form-item label="选择配料" required :class="{ 'error-input': formErrors.ingredients }">
            ...
            <el-button type="primary" @click="openIngredientDialog" style="margin-top: 10px">添加配料</el-button>
          </el-form-item>
          ...
          <el-form-item>
            <div class="form-actions">
              <el-button type="primary" native-type="submit" :loading="loading">更新菜品</el-button>
              <el-button @click="navigateTo('/dishes')">取消</el-button>
            </div>
          </el-form-item>
        </el-form>
      </div>
    ...
    >
      <div class="dialog-content">
        <div class="filter-section">
          <el-select v-model="selectedCategory" placeholder="按分类筛选" style="width: 200px;">
...
onMounted(async () => {
  ...
  const [dishResponse, ingredientsResponse, categorySettingsResponse, kitchenwaresResponse] = await Promise.all([
    dishId ? dishStore.fetchDishById(dishId) : Promise.resolve(),
    ingredientStore.fetchIngredients(),
    categorySettingsApi.getCategorySettings(),
    kitchenwareApi.getKitchenwares()
  ]);
      ...
      if (dish.dish_ingredients) {
        ...
        selectedIngredients.value = dish.dish_ingredients.map(di => {
          ...
          return {
            ...
            category: di.ingredient?.category || di.Ingredient?.category || '',
            unit: di.ingredient?.unit || di.Ingredient?.unit || '',
...
const openIngredientDialog = () => {
  ...
  // 将已选用料的ID添加到选中数组中
  selectedIngredientIds.value = selectedIngredients.value.map(ingredient => ingredient.id);
...
const updateDish = async () => {
  ...
  try {
    ...
    await dishStore.updateDish(dishId, dishData);
    router.push('/dishes');
  } catch (error) {
    console.error(error);
  } finally {
