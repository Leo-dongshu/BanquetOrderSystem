...
const fetchKitchenwareDetails = async () => {
  const id = route.params.id as string;
  try {
    const response = await kitchenwareApi.getKitchenwareById(Number(id));
    const kitchenware = response.data;
    form.name = kitchenware.name;
    form.type = kitchenware.type;
    form.quantity = kitchenware.quantity;
  } catch (error) {
    console.error('获取厨具详情失败:', error);
    ElMessage.error('获取厨具详情失败');
  }
};
...
const submitForm = async () => {
  ...
  try {
    ...
    const id = route.params.id as string;
    await kitchenwareApi.updateKitchenware(Number(id), {
      name: form.name,
      type: form.type,
    ...
    console.error('保存厨具失败:', error);
    ElMessage.error('保存厨具失败');
  }
};

onMounted(() => {
  fetchCategories();
  fetchKitchenwareDetails();
});
</script>
