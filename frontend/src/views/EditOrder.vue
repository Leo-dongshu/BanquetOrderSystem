  ...
  <div class="edit-order">
    <el-card>
      ...
      <div class="edit-order-content">
        <el-form :model="orderForm" label-width="120px" @submit.prevent="updateOrder">
          <el-form-item label="订单编号" required>
            <el-input v-model="orderForm.order_number" placeholder="系统生成" disabled />
          </el-form-item>
            ...
            <div class="meal-info-table">
              <el-table :data="[{}]" style="width: 100%" :fit="true" v-if="orderForm.set_meal_id">
                ...
                <el-table-column label="正式桌" min-width="100" align="center">
                  <template #default>
                    <el-input-number v-model="orderForm.formal_tables" :min="0" placeholder="正式桌" :class="{ 'error-input': formErrors.formal_tables && orderForm.formal_tables === 0 }" />
                  </template>
                </el-table-column>
          ...
          <el-form-item>
            <div class="form-actions">
              <el-button type="primary" native-type="submit" :loading="loading">更新订单</el-button>
              <el-button @click="navigateTo('/orders')">取消</el-button>
            </div>
          </el-form-item>
        </el-form>
      </div>
...
onMounted(async () => {
  ...
  if (orderId) {
    ...
    if (order) {
      ...
      orderForm.customer_phone = order.customer_phone;
      orderForm.customer_phone2 = order.customer_phone2 || '';
      orderForm.service_address = order.service_address;
      orderForm.service_date = order.service_date;
      orderForm.region = order.region;
      orderForm.source = order.source;
      orderForm.receiver_id = order.receiver_id;
      ...
      orderForm.deposit = order.deposit;
      orderForm.payment_method = order.payment_method;
      orderForm.remark = order.remark || '';
      orderForm.formal_tables = order.formal_tables;
      orderForm.backup_tables = order.backup_tables;
      
      // 处理区域值
      if (order.region) {
        regionValue.value = order.region.split(' / ');
      }
      
      // 加载调整后的菜品列表
      if (order.order_dishes) {
        const dishIds = order.order_dishes.map(od => od.dish_id || od.dish?.id || od.Dish?.id || 0).filter(id => id > 0);
        adjustedDishes.value = allDishes.value.filter(dish => dishIds.includes(dish.id));
      }
      
      // 加载套餐详细信息
      if (order.set_meal) {
        const dishes = order.set_meal.dishes?.map(d => ({
          dish_id: d.dish_id,
          quantity: d.quantity
        })) || [];
        
        selectedSetMealInfo.value = {
          id: order.set_meal.id,
          name: order.set_meal.name,
          price: order.set_meal.price,
          dishes: dishes
        };
      }
    }
...
const handleSetMealChange = (setMealId: number | undefined) => {
  if (setMealId) {
...
const adjustDishes = async () => {
  ...
  // 如果没有预存储的菜品列表，加载后端订单菜品表保存的菜品列表
  if (adjustedDishes.value.length === 0) {
    try {
      const orderId = Number(route.params.id);
      if (orderId) {
        // 重新获取订单数据，确保包含最新的菜品信息
        await orderStore.fetchOrderById(orderId);
        const order = orderStore.currentOrder;
        if (order && order.order_dishes) {
          const dishIds = order.order_dishes.map(od => od.dish_id || od.dish?.id || od.Dish?.id || 0).filter(id => id > 0);
          adjustedDishes.value = allDishes.value.filter(dish => dishIds.includes(dish.id));
        } else if (orderForm.set_meal_id) {
          // 如果订单中没有菜品信息，加载选择套餐的菜品列表
